...
import happySongs from "./model/happy";
...
export default function Music() {
    ...
    async function loadAudio() {
        try {
            const { sound } = await Audio.Sound.createAsync(happySongs[songIndex].url);
            setSound(sound);
    ...
    async function handleNext() {
        ...
        const nextSongIndex = songIndex + 1 >= happySongs.length ? 0 : songIndex + 1;
        setSongIndex(nextSongIndex);
        const { sound: nextSound } = await Audio.Sound.createAsync(happySongs[nextSongIndex].url);
        setSound(nextSound);
    ...
    async function handlePrevious() {
        ...
        const previousSongIndex = songIndex - 1 < 0 ? happySongs.length - 1 : songIndex - 1;
        setSongIndex(previousSongIndex);
        const { sound: nextSound } = await Audio.Sound.createAsync(happySongs[previousSongIndex].url);
        setSound(nextSound);
        soundRef.current = nextSound; // atribui o objeto de áudio ao ref
        playSound();
    }
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.touchableContainer}>
                ...
                {/* image */}
                <Animatable.Image style={styles.artwork} source={happySongs[songIndex].artwork } animation='fadeInDown' />

                {/* Song Content */}
                <Animatable.View animation='fadeInRight'>
                    <Text style={[styles.songContent, styles.songTitle]}> {happySongs[songIndex].title} </Text>
                    <Text style={[styles.songContent, styles.songArtist]}> {happySongs[songIndex].artist} </Text>
                </Animatable.View>
